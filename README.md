...
**Agent 14 — Rebalance Best-Execution Strategist:** answers the client question the event study sets up — *how should this flow actually be executed?* Simulates four literature-anchored strategies on the event's **actual** price/volume path at day-level granularity: **S1** tracker baseline (100% in the effective-day closing auction — zero benchmark risk, maximum crowding), **S2** pre-position after the announcement with the remainder at the close (Madhavan 2003), **S3** partial print + post-effective completion capturing reversal of the temporary price-pressure component (Harris & Gurel 1986), and **S4** announcement-anchored spread (the classic "index game" — with Greenwood & Sammon's 2025 caveat that this alpha has compressed). Each strategy is scored on the two dimensions institutional desks trade off: implementation cost vs. the pre-announcement decision price and tracking difference vs. the effective close, plus closing-auction stress vs. observed event-day volume and square-root impact at daily participation. Renders the cost-vs-tracking frontier and an objective-aware recommendation (Index Tracker vs. Cost-Minimizing mandate); both sides (addition = buy, deletion = sell) supported. Evidence base with citations: `docs/INDEX_REBALANCE_RESEARCH.md`. Honest scope: ex-post case study on one name's realized path — not an expected-value forecast — with all modeling assumptions disclosed in-app.

**Agent 12 — Rebalance Calendar Monitor (auto-fetched real index changes):** instead of manually typing a ticker and effective date, Page 2 can pull real, current constituent adds/deletes from the three major index providers' public announcement pages and load one into the event-study inputs with a click. Each provider publishes differently, so each gets its own adapter (failures degrade independently, same philosophy as the main pipeline):

| Provider | Method |
|---|---|
| MSCI | Structured public announcement feed (app2.msci.com) — list page → per-security detail pages with labelled fields (country, security, add/delete, effective date). Fully parsed. |
| FTSE Russell | LSEG's press-release listing is JS-rendered, so candidate URLs are *constructed from the review calendar* (quarterly reviews Mar/Jun/Sep/Dec, known slug patterns), then the FTSE 100/250 additions/deletions tables are parsed from each release that exists. |
| S&P DJI | PR Newswire company feed — release pages carry a summary table (Effective Date / Index / Action / Company / Ticker / GICS sector), fully parsed, with a sentence-level "X will replace Y in the S&P 500" regex fallback. |

Where the provider doesn't publish a ticker (MSCI, FTSE), a best-effort Yahoo symbol lookup suggests one, filtered to the expected exchange suffix — always editable before running. The section also shows each provider's approximate upcoming review/rebalance calendar. Data is fetched **on demand** (a manual refresh button) and cached to `data/index_changes_cache.json`; a scheduled GitHub Actions workflow (`.github/workflows/refresh-index-changes.yml`) can refresh the cache once per weekday for demo purposes and is designed to be switched off afterwards — these are public pages meant for manual reading, not APIs, so fetching is deliberately low-volume.
