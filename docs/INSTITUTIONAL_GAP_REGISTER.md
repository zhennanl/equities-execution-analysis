...
| **Program trading desk (Asia)**: market session board (time zones/lunch/close), per-market regulation reference (board lots, short-sale regimes, circuit breakers, T+n settlement), program pre-trade blotter with lot/short/capacity flags, cross-market execution wave plan, simulated EOD reconciliation | `agents/program_trading.py` + Page 3 — shipped 2026-07-08 (program-trader JD mapping, session 5j) |
...
| Regression test suite + CI over the analytics kernels (pins the hand-verified anchors: cap→fill, limit→opp cost, wick convention, S1-S4 costs, YZ/CS/AR estimators, routing, explicit costs, alerts) | `tests/` (offline pytest, 178 tests as of 2026-07-08 + recorded AAPL integration fixture) + `.github/workflows/tests.yml` — shipped 2026-07-08 |
...
| I-5 ✅ *(shipped 2026-07-08: `agent6.build_is_attribution` — Perold delay/trading/opportunity/explicit reconciling ±0.1bp by construction; waterfall UI; modeled impact shown as memo)* | **Full IS attribution** in post-trade: delay cost / trading cost / opportunity cost / explicit fees, reconciling to total shortfall | Standard TCA decomposition (Perold framework) | Mostly re-arrangement of existing numbers + a fee model (I-6) |
...
| I-7 ✅ *(shipped 2026-07-08: `agents/algo_wheel.py` — Friedman + Nemenyi CD league table on the day-blocked cost matrix; Page-1 section)* | **Algo wheel module**: randomized strategy assignment across historical days, league table with statistical significance | Broker algo wheels | Generalize Agent 10's paired framework to N-arm randomized comparison |
| I-8 ✅ *(shipped 2026-07-08: EMS-style order-detail expander — child slices + cumulative completion + schedule table)* | **Parent/child order visualization**: planned vs. actual execution schedule timeline, child slices with venue tags | EMS order detail view | Plotly timeline of Agent 3/11 slice data (+ I-2 venue tags) |
