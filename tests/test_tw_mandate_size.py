...
2. THE FLOOR QUIETLY STOPS BEING A FLOOR. The construction is
   defensible only because each step errs downward: mandates
   priced at the ETF fee rate, capped funds priced at uncapped
   weights, and no gross-up for the ETFs nobody enumerated. If a
   later edit swaps in a "more realistic" fee rate, the number
   becomes an estimate wearing a floor's clothes and the page's
   language is then wrong.
...
def test_the_non_etf_pool_is_priced_at_the_etf_rate(d):
    """THE CONSERVATIVE CHOICE, PINNED.

    Institutional mandates pay less per dollar than retail ETFs,
    so pricing them at the ETF rate understates their assets. A
    lower rate here would produce a bigger, less defensible
    number — this asserts nobody has done that."""
    n = d["non_etf_indexed"]
    assert (n["etf_effective_bp_annualised"]
            >= d["msci_disclosure"]["etf_bp_fee_period_end"] - 0.25)
    implied = (d["msci_disclosure"]["abf_non_etf_indexed_usd_m"]
               * 4 / (n["etf_effective_bp_annualised"] / 1e4) / 1e3)
    assert abs(implied - n["non_etf_indexed_aum_floor_usd_b"]) < 0.5
    # a third of the ETF pool, not a multiple of it
    assert 0.2 < n["multiplier_floor"] < 0.6, n["multiplier_floor"]
    # the reason has to name the DIRECTION of the error, not
    # just assert that one exists
    why = n["why_it_is_a_floor"].lower()
    assert "less per dollar" in why
    assert "too low" in why or "too small" in why
