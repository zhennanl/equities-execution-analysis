...
def _step1_diagrams(d):
    """The same two SVGs the page draws (c-249).

    Imported lazily: this module is also run headless by
    `write()`, and `views.diagrams` pulls the palette out of
    `views.design`, which imports streamlit.
    """
    try:
        ...
        return ""
    return (diagrams.review_flow(
                announced=_human(d.get("announced")),
                close=_human(d.get("rebalance_close")),
                announced_time=d.get("announced_time_lines"))
            + "<p class='cap'>Source: MSCI Review Dates and the "
              "August 2026 Announcement.</p>")


def _md(p):
    """Inline markdown -> HTML. The story text is written in
    markdown for Streamlit; the export used to escape it wholesale,
    which printed literal ** and - to anyone reading the saved
    file. Bold, links and bullets only — nothing else is used.
    Everything is escaped FIRST, so this adds no injection path.
    """
    import re
    if not p.strip():
        return ""
    bullet = p.lstrip().startswith("- ")
    t = _h.escape(p.lstrip()[2:] if bullet else p)
    t = re.sub(r"\[([^\]]+)\]\((https?://[^)]+)\)",
               r"<a href='\2'>\1</a>", t)
    t = re.sub(r"\*\*([^*]+)\*\*", r"<b>\1</b>", t)
    t = re.sub(r"\*([^*]+)\*", r"<i>\1</i>", t)
    return f"<li>{t}</li>" if bullet else f"<p>{t}</p>"
        ...
        if stp["numbers"]:
            ...
            for n in stp["numbers"]:
                ...
                out.append(
                    ...
                    f"</div>{nt}</div>")
            out.append("</div>")
        # c-249: step 1's two diagrams travel with the file.
        # The export's promise is that it carries what the page
        # carries; shipping the prose without the drawings that
        # replaced two of its paragraphs would break that.
        if stp["n"] == 1 and (s.get("dates") or {}).get(
                "rebalance_close"):
            out.append(_step1_diagrams(s["dates"]))
        for p in stp["plain"]:
            out.append(_md(p))
        if stp["n"] == 5:
            out.append(_chart(s))
            out.append(
                ...
                "at the Price Cutoff Date. The blue line is "
                "MSCI's "
                ...
                "sides.</p>")
        # c-162: the "For the desk" and "what this step can
        # get wrong" blocks are gone from the story. Their
        # content now lives in the main text (desk language) and
        # in step 7 (the limits), so there is nothing to render.
        out.append("</div>")
    out.append(
